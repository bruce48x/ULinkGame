...
summary: "ULinkGame 在 ULinkRPC 之上补齐在线游戏常见的会话、宿主、重连和可靠业务推送基础设施。"
---

ULinkRPC 负责强类型 RPC、生成代码、transport、serializer 和 callback。ULinkGame 站在更高一层，关注在线游戏经常重复实现的会话基础设施。
...
`ULinkGame.Server` 用在 .NET 服务端进程中，提供 ULinkRPC hosting、Orleans integration 和 reliable push outbox。
...
生成的项目会先使用 ULinkRPC starter 创建基础 RPC 项目，再补充 ULinkGame server hosting、Orleans 配置和可靠业务推送基础设施。
