...
`ULinkGame.Server` 是“在线游戏服务宿主”，不是“游戏业务大全框架”。
...
`ULinkGame.Server` 只应补足它们之间的服务端宿主接缝。
...
任何新能力进入 `ULinkGame.Server` Core 前，必须先通过这两个判断：
...
而不是放进 `ULinkGame.Server`。

### 4. 框架定义运行与集成方式，不定义业务组织方式

`ULinkGame.Server` 不强制规定：
...
当前 `ULinkGame.Server` 中，属于 Core 的内容包括：
...
后续对 `ULinkGame.Server` 的任何新增代码，都应遵守以下规则：
...
未来 `ULinkGame.Server` 可以继续向这些方向演进：
...
`ULinkGame.Server` Core 必须能够灵活支持任意 `ULinkRPC.Transport` 与任意 `ULinkRPC.Serializer`。

因此：

- `ULinkGame.Server` 不应内置或硬编码特定 serializer
- `ULinkGame.Server` 不应内置或硬编码特定 transport
- `ULinkGame.Server` 只负责托管 RPC server 生命周期
...
配置接口接收 `ULinkGameServerRpcContext`，业务项目可以通过其中的 `Builder`
配置 serializer / transport / service binder，也可以通过 `Services` 接入宿主
DI 容器创建 RPC service。

`ULinkGame.Server` Core 不预设游戏一定存在“控制面 + 实时面”，也不预设客户端一定
...
这样 `ULinkGame.Server` 保持宿主层中立，而业务项目可以自由选择：
