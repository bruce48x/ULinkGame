...
The tool creates a ULinkRPC-based project, adds ULinkGame server/client integration, prepares the server hosting code, and includes explicit cluster configuration scaffolding by default. RPC glue is generated by `ULinkRPC.Analyzers` during normal server builds and Unity/Godot client compilation, so you do not need to commit generated RPC source.
