...
- A game needs explicit cluster-ready server infrastructure without rebuilding the project layout later. How do you keep that foundation visible from day one?
...
The tool creates a ULinkRPC-based project, adds ULinkGame server/client integration, prepares the server hosting code, and includes Cluster, Hotfix, and Reliable Push infrastructure by default. Validate the generated server state before starting it:
...
RPC glue is generated by `ULinkRPC.Analyzers` during normal server builds and Unity/Godot client compilation, so you do not need to commit generated RPC source.
