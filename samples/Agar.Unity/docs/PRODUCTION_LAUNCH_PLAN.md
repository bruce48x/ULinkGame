...
`PlayerService`、`SessionRegistration` 和 `SessionDirectory` 已接入 `ULinkGame.Server.Sessions`，可靠推送 ack 已接入框架 outcome。上线前仍必须集中验证这些路径和 `DisconnectedSessionCleanupHostedService`、`RoomRuntimeHost`、`RoomRuntime` 的业务清理语义。
