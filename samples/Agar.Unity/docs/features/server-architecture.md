...
- 周一 UTC 00:00 触发重置：清空排行榜索引内用户的 `VictoryPoints`，归档上周数据。
- 排行榜 grain 自身状态存储当前周期标识（`CurrentPeriodStartUtc`）、当前周期积分索引和最近两周快照。当前没有全量用户目录，因此重置覆盖的是索引内出现过的玩家；如果后续新增用户目录，需要扩展为全用户重置。
