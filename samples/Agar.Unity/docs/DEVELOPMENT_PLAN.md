...
- 排行榜周期重置的目标口径已改为榜单当地时间周一 00:00；当前实现仍使用 UTC 周期字段和 UTC 周一 00:00 判断，需要在阶段 4 修正。
...
当前玩家球方向已通过单机初验，已进入第一轮基础美术资产批量生成。所有资产生成后都需要先由 MCP for Unity 完成导入和基础运行路径验证，再交给用户手动验收：
...
- 新建 `Server/Silo/Leaderboard/LeaderboardGrain.cs`：实现 `ILeaderboardGrain`（`IGrainWithIntegerKey`，固定 key=0，单实例），负责：
  ...
  - 记录当前周期标识（`yyyy-MM-dd` 格式的周一日期），每次查询或写入时检查是否已过周一 UTC 00:00，若是则先执行重置。（已完成旧实现；需改为榜单当地时间周一 00:00）
...
- 将 `LeaderboardGrain` 的周期计算从 UTC 周一 00:00 改为榜单当地时间周一 00:00：
  - 增加或集中定义榜单时区配置，当前样例默认使用目标玩家/部署地区的本地时区。
  - 服务端可继续使用 UTC 存储绝对时间戳，但 `CurrentPeriodStart`、`SecondsUntilReset` 和归档周期标识必须按榜单当地时区计算。
  - 如果未来支持多地区排行榜，每个地区榜独立保存时区和周期。
- 调整接口和客户端文案中 `PeriodStartUtc` 这类命名，避免把体验口径继续表达为 UTC；如需兼容旧字段，应在计划中明确迁移策略。
