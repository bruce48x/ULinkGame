...
- 以 [`ART_DIRECTION.md`](ART_DIRECTION.md) 作为美术生产基准，但第一步只处理玩家球图片，不同时推进食物、场地、UI 和特效。
...
- 用户看过玩家球效果并确认方向后，再进入下一步资产生成。后续候选顺序为：质量拾取物、竞技场背景、UI 面板/按钮、吸收和复活特效。
...
- `samples/Agar.Unity/docs/ART_DIRECTION.md` 已作为后续资产生成和筛选的唯一风格标准。
