...
新的方向是自底向上重建。ULinkGame 不再把 Orleans 当作分布式运行时，而是以独立发布的 `ULinkActor` 作为面向游戏服务器的小型 actor/mailbox 运行时根基，并在它之上提供游戏会话、RPC hosting、可靠推送、路由和部署组织能力。

`ULinkActor` 是同一产品线下的独立项目，已发布到 NuGet：

```text
https://www.nuget.org/packages/ULinkActor/
```

保持独立不是临时拆包，而是长期边界：`ULinkActor` 的生命周期可以长于 ULinkGame；如果 actor/mailbox 运行时有不适合游戏场景的能力或缺口，也可以直接在 `ULinkActor` 中优化，而不需要把 ULinkGame 变成厚重运行时仓库。
...
- 用独立的 `ULinkActor` actor/mailbox runtime 加 ULinkGame 服务端路由、会话和可靠推送层替换 Orleans。
...
- `ULinkActor` 的长期目标是 skynet 风格的轻量工程化消息运行时，而不是 Orleans/Akka 风格的重型通用 actor framework；ULinkGame 不重新拥有一套 actor runtime。
...
其中 `Actor`、`Mailbox` 和 `Timer` 属于 `ULinkActor` 的轻量 actor/mailbox core。`Router`、`Location` 和 `Internal RPC abstraction` 属于 ULinkGame 服务端的集群和路由集成层，建立在 `ULinkActor` 之上。`ComponentState`、`ComponentSystem`、Persistence、Hotfix 和 Codegen 是运行时之上的工程能力，不属于 `ULinkActor` Core 的定义本身。
...
轻量 Actor Core 由 `ULinkActor` 拥有并长期保持轻量。目标是类似 skynet 的工程化 actor/service 运行时，而不是理论上完整、通用、可透明分布的 actor 模型。
...
- 管理 Timer 和 Tick 入口；
- 显式 `send` / `call`；
- 暴露本地 mailbox 背压、超时、dead letter、慢消息和诊断。

ULinkGame 服务端集成层负责：

- 通过 Location 解析远程目标；
- 通过 Transport 发送跨 Node 消息；
- 把跨节点超时、过载、stale route、no owner 等失败结果显式暴露给上层；
- 把 actor 执行与 session、endpoint binding、reliable push 和 reconnect 语义连接起来。
...
- `ULinkActor`：独立 actor/mailbox runtime，负责 Actor、ActorRef、Mailbox、Timer、Backpressure、Diagnostics 和 Source Generator。
- `ULinkGame.Server`：基于 `ULinkActor` 的服务端游戏基础设施，负责 ULinkRPC hosting 集成、会话生命周期、可靠推送、actor RPC 路由和诊断。
...
服务端运行时使用以下层级。进程内 actor/mailbox 层由 `ULinkActor` 提供，ULinkGame 在其上连接会话、RPC hosting、路由和可靠推送：
...
`ULinkActor` Core 的最小组成是：

```text
Actor = ActorId + Mailbox + Handler + Context
```

业务可以在 Actor 内组合 ComponentState、Entity、Persistence、Hotfix handler 或 generated stub，但这些都是 `ULinkActor` Core 之上的工程能力，不改变 `ULinkActor` Core 的轻量边界。
...
- `ActorId` 由 `ULinkActor` 或 ULinkGame 上层路由适配提供的 `IActorIdGenerator` 分配，业务层不直接拼接 Id。
...
进程内 actor runtime API 属于 `ULinkActor`，应该刻意保持很小，并以显式消息为核心。ULinkGame 不重新定义另一套并行 runtime，只在需要跨节点、会话和可靠推送集成时提供上层适配：
...
`ULinkActor` 只提供轻量运行时和生命周期容器能力。游戏可以在业务层定义 `WorldScene`、`DungeonScene`、`BattleRoom`、`MatchLobby` 等根对象，ULinkGame 负责把这些对象接入会话、RPC hosting 和可靠推送。
...
src/
  ...
  ULinkActor/                 // separate repository/package
    Actor/
    Mailbox/
    Timers/
    ActorRuntime/
...
`ulinkgame-tool new` 应该生成基于 `ULinkActor` 包的 actor-host 项目：
...
- 以 `ULinkActor` NuGet 包为根基验证单进程 actor host；
- 验证 Actor mailbox；
- 验证 timer；
- 验证显式 Send/Call；
...
- 明确 Hotfix 是 `ULinkActor` Core 之上的工程能力，不反向污染 `ULinkActor` Core。
...
1. 从框架包中移除 Orleans。
2. 引入独立 `ULinkActor` / `ULinkActor.SourceGenerator` NuGet 包。
...
- 优先 `ULinkActor` 本地 actor runtime，然后 distributed RPC loopback，然后一个生产 transport。
